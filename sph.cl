...
kernel void compute_density_and_pressure(global float* density,
    ...
    float square, sum = 0, kernel_radius_squared = kernel_radius * kernel_radius;
    float2 delta, cur_pos = positions[glob_id];
    ...
    density[glob_id] = sum;
    pressure[glob_id] = gas_const * (density[glob_id] - rest_density);
}
...
kernel void compute_forces(global float* density,
    ...
    float2 delta, cur_pos = positions[glob_id], cur_forces = forces[glob_id],
            cur_velocity = velocities[glob_id];
    ...
    float2 gravity_force = G * density[glob_id];
